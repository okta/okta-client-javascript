...
const assertTagsValues = async (tabs) => {
  const tags = [];
  await assertAllTabs(tabs, async () => {
    const tag = await RedirectModelApp.tags.getText();
    expect(tag).toBeDefined();
    tags.push(tag);
  });
  expectSameValue(tags);
  return tags;
};
...
describe('Cross Tab Sync' , () => {
  ...
  it('can sync Credential.getDefault after adding', async () => {
    // load multiple tabs
    const tabs = await bootstrap();
    await assertAllTabs(tabs, assertFreshPage);
    ...
    // delete the default token (via Credential.getDefault)
    ...
    // const defaultId = await browser.executeAsync(function (done) {
    //   window.Credential.getDefault().then(cred => done(cred.id || 'null'));
    // });
   ...
   // changes the default value manually (without removal)
    ...
    // assert all tabs know of the new default
    const newDefaultId = await browser.executeAsync(function (done) {
      window.Credential.getDefault().then(cred => done(cred.id || 'null'));
    });
  ...
  it('can sync tags across tabs', async () => {
    // load multiple tabs
    const tabs = await bootstrap();
    await assertAllTabs(tabs, assertFreshPage);
    ...
    await RedirectModelApp.withTagLink.click();
    await performSignIn();
    await RedirectModelApp.backToHome.click();
    ...
    const tags1 = (await assertTagsValues(tabs))[0];

    let tags2;
    await RedirectModelApp.addTagBtn.click();
    await browser.waitUntil(async () => {
      tags2 = (await assertTagsValues(tabs))[0];
      return tags2 !== tags1;
    }, { timeout: 5000, timeoutMsg: 'wait for tag value to change after addTagBtn click' });

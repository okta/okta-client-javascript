...
describe('Token', () => {
  ...
  describe('instantiate', () => {
    ...
    it('should return Token instance with generated id', () => {
      ...
      const randomValuesSpy = jest.spyOn(global.crypto, 'getRandomValues');
    ...
    describe('get expiresAt', () => {
      ...
      it('should throw, no setter', () => {
        ...
        expect(() => token.expiresAt = new Date())
          .toThrow('Cannot set property expiresAt of #<Token> which has only a getter');
      });
    });
  ...
  describe('methods', () => {
    ...
    describe('.authorize', () => {
      it('will authorize a request using bearer tokens', async () => {
        ...
        const bc1 = await t1.authorize('http://localhost:8080/foo', { headers: { foo: 'bar' }});
      ...
      it('will authorize a request using dpop-bound tokens', async () => {
        ...
        // called with fetch signature (url, RequestInit)
        const bc1 = await token.authorize('http://localhost:8080/foo', { headers: { foo: 'bar' }});
        expect(bc1).toBeInstanceOf(Request);
        expect(Object.fromEntries(bc1.headers.entries())).toEqual({
          foo: 'bar',
        ...
        const bc3 = await token.authorize('http://localhost:8080/foo', { headers: { foo: 'bar' }, dpopNonce: 'nonceuponatime' });
  ...
  describe('namespace methods', () => {
    it('extractContext', () => {
      ...
      const tokenContext = {
        ...
        dpopPairId: 'somekeypairid'
      };

      expect(Token.extractContext(tokenContext)).toEqual(tokenContext);
      expect(Token.extractContext({ ...tokenContext, foo: 1, bar: 1 })).toEqual(tokenContext);
      expect(Token.extractContext({ ...tokenContext, acrValues: undefined, dpopPairId: undefined })).toEqual(required);
    });
  });
});

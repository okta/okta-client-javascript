...
async function openAuthSessionIOS (
  ...
  return await NativeBrowserSessionBridgeSpec.openAuthSession(url, redirectScheme);
}
...
async function openAuthSessionAndroid (
  ...
  const browserPromise = NativeBrowserSessionBridgeSpec.openBrowser(url)
  .then(() => {
 ...
 *
 * try {
 *   const result = await openAuthSession(authUrl, redirectUri);
...
export async function openAuthSession(
  ...
  switch (Platform.OS) {
    case 'ios':
      return openAuthSessionIOS(url, redirectUri);
    case 'android':
      return openAuthSessionAndroid(url, redirectUri);
    default:

...
foreach_workspace () {
  ...
  # gets list of all workspace directories relative to project directory
  local workspaces=$(yarn -s workspaces info | jq 'map(..|objects|select(.location).location) | @sh' | tr -d \'\")
